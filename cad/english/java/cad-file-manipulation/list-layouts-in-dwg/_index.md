---
title: Extract Layout Information from CAD Files Using Java
...
If you’re looking for a reliable way **how to read dwg** files programmatically, Aspose.CAD for Java provides a clean, cross‑platform API that lets you load a drawing and pull out any information you need—such as the names of all layouts inside the file. In this step‑by‑step tutorial we’ll show you **how to read DWG** and list every layout contained in a DWG (or DXF) file, so you can integrate this capability into any Java application that works with CAD data.

## Quick Answers
- **What library is required?** Aspose.CAD for Java.  
- **Can I read DWG files on any OS?** Yes – Java is cross‑platform, so you can **read dwg on linux** as easily as on Windows.  
...
Reading a DWG file means loading the binary CAD data into an object model that you can query. Aspose.CAD abstracts the complex DWG structure behind simple Java classes, allowing you to focus on the information you need – in this case, layout names.
...
Replace **“Your Document Directory”** with the absolute path where your CAD files reside. On Linux you might use a path like `/home/user/cad/`.
...
The `Image.load` method detects the file format automatically, so the same code works for both **DWG** and **DXF** files.
...
If you later need to turn a specific layout into a PDF, Aspose.CAD can render the layout to an image and then you can use Aspose.PDF (or any other PDF library) to embed that image into a PDF document. The conversion code is identical on Linux because the API is pure Java.
...
A2: Yes, you can obtain a free trial from the [Aspose.CAD free trial page](https://releases.aspose.com/).
...
A5: Yes, you can obtain a temporary license from the [temporary license page](https://purchase.aspose.com/temporary-license/).
