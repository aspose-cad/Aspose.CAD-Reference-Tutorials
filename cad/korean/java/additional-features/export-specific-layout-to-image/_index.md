...
- **Aspose.CAD for Java**가 설치되어 있는지 확인하세요([Aspose CAD Java 다운로드 페이지](https://releases.aspose.com/cad/java/)에서 다운로드).
- Java 개발 환경 (JDK8 이상).
-변환 작업을 포함하는 DXF(또는 DWF) 파일입니다.

## 투명 비즈니스 가져오기

CAD 파일과 상호 작용하려면 필수 클래스를 가져옵니다.
